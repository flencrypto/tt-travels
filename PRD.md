...
### Multi-Destination Route Planner with Optimization
- **Functionality**: Plan comprehensive routes across multiple destinations with automatic geocoding, distance calculations, travel time estimates, intelligent route optimization, and interactive map visualization. Users can add unlimited stops, reorder destinations manually or automatically optimize them, use current location as starting point, and view detailed breakdowns of each route segment with appropriate transport modes (driving vs. flight)
- **Purpose**: Help travelers plan complex multi-stop journeys by providing accurate distance and time calculations, minimize total travel distance and time through intelligent route optimization, enabling better itinerary planning and efficient journeys across multiple destinations
- **Trigger**: Navigate to Route Planner page (/route-planner), add destinations (minimum 2), click "Calculate Route", optionally click "Optimize Route Order" for 3+ destinations
- **Progression**: User lands on route planner → enters destination names (minimum 2, unlimited maximum) → optionally uses "Use Current Location" button to set starting point via browser geolocation → adds more destinations using "Add Destination" button → reorders stops using up/down arrow controls → selects distance unit preference (metric/imperial) → clicks "Calculate Route" → system geocodes all destinations using Open-Meteo Geocoding API → calculates distances between consecutive stops using Haversine formula → determines transport mode for each segment (driving for <500km, flight for ≥500km) → estimates travel time (80km/h for driving, 800km/h for flights) → displays route summary with total distance and total travel time → shows detailed breakdown of each segment with from/to locations, distance, duration, and transport mode badges → FOR ROUTES WITH 3+ DESTINATIONS: "Optimize Route Order" button appears → clicking optimization triggers nearest-neighbor algorithm that keeps start and end points fixed while reordering middle waypoints to minimize total travel distance → optimized route recalculates all segments and displays savings in distance and time → "Optimized" badge appears in route summary → interactive map visualization shows all destinations with numbered markers and connecting route lines → user can modify destinations and recalculate or clear entire route to start fresh
- **Success criteria**: Successfully geocodes all destination names within 5 seconds, accurate distance calculations using spherical coordinate Haversine formula, appropriate transport mode selection based on distance thresholds, travel time estimates reflect realistic speeds for each mode, route segments display in order with clear visual hierarchy, total distance and time calculations are accurate sums of all segments, **optimization algorithm preserves first and last destinations while reordering middle stops using nearest-neighbor approach**, **optimization completes within 3 seconds for routes up to 20 destinations**, **displays clear before/after comparison showing distance and time saved**, **"Optimized" badge visually indicates when route has been optimized**, **optimization button only appears when route has 3+ destinations and has been calculated**, **helpful tooltip explains optimization keeps start/end points fixed**, distance displays in user's preferred units (km or miles), travel time formats clearly with hours and minutes, supports minimum 2 destinations up to 20+ stops, reordering destinations manually clears optimization flag prompting re-optimization, "Use Current Location" successfully retrieves browser geolocation and reverse geocodes to display readable location name, clear empty state shown before route calculation, responsive design works on mobile and desktop, smooth animations for adding/removing destinations, error handling for failed geocoding with specific destination feedback, prevents calculation with empty destination fields, **proper handling when optimization yields same or better route with appropriate success message**
